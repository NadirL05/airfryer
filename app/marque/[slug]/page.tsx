...
import { getBrands, getFilteredProducts, type ProductFilterOptions } from "@/lib/supabase/queries";
...
function parseFiltersFromSearchParams(
  ...
  return {
    ...
    brandIds: [brandId], // Filter by this brand
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!brand) {
    return {
      title: "Marque non trouvée | AirFryer Deal",
    };
  }

  const title = `Air Fryers ${brand.name} - Tests & Comparatifs 2026 | AirFryer Deal`;
  const description = `Découvrez tous les air fryers ${brand.name}. Comparez les modèles, les prix et les fonctionnalités pour trouver le meilleur ${brand.name}.`;
...
async function ProductGridLoader({ brandId, filters }: ProductGridLoaderProps) {
...
export default async function BrandPage({ params, searchParams }: PageProps) {
  ...
  const filters = parseFiltersFromSearchParams(sp, brand.id);
  const activeFilterCount = countActiveFilters(sp);

  // Transform brands for sidebar (excluding current brand from filter)
  const availableBrands: Brand[] = brands.map((b) => ({
    id: b.id,
    name: b.name,
    slug: b.slug,
    logo_url: b.logo_url,
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="space-y-2">
          <h1 className="text-3xl font-bold tracking-tight sm:text-4xl lg:text-5xl">
            Air Fryers {brand.name}
          </h1>
          <p className="max-w-2xl text-sm text-muted-foreground sm:text-base">
            Découvrez tous les modèles d&apos;air fryers {brand.name}. Comparez les caractéristiques, les prix et les avis pour trouver le modèle idéal.
          </p>
        </div>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-12">
        ...
        <section className="lg:col-span-9">
          <Suspense key={suspenseKey} fallback={<ProductGridSkeleton count={12} />}>
            <ProductGridLoader brandId={brand.id} filters={filters} />
          </Suspense>
        </section>
      </div>
    </div>
