-- ============================================
-- MIGRATION: Fix get_filtered_products RPC (remove product_categories dependency)
-- Applied: 2026-02-02
-- Description:
--   La table product_categories n'existe pas, donc la fonction échouait.
--   Cette migration simplifie la fonction pour utiliser uniquement products.type
-- ============================================
...
BEGIN
  ...
  WITH filtered_products AS (
    ...
    WHERE
      ...
      -- Category filter (uses products.type field: compact, family, xxl)
      AND (
  ...
  SELECT
    ...
    s.prod_brand_name,
    s.prod_brand_slug,
    s.prod_brand_logo_url,
    s.prod_short_description,
    s.prod_min_price,
    s.prod_max_price,
    s.prod_capacity_liters,
    s.prod_type,
    s.prod_rating_overall,
    s.prod_main_image_url,
    s.prod_has_dual_zone,
    s.prod_has_app,
    s.prod_has_rotisserie,
    s.prod_has_dehydrator,
    s.prod_specs,
    s.prod_created_at,
    s.cnt
  FROM sorted s
